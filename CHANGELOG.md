...
- The residual loop between the SIMD/SWAR skip and the tail stays 8 bytes per iteration: a 16-byte-pair variant was tried and regressed Neoverse-V2 by 10–40% at 8–128 B in a same-box A/B (the extra tail branches cost more than the saved test), while being statically dead on x86-64's 16-byte skip.
